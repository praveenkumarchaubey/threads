...
Create a library of this message queue. There should be a header file msgqueue.h which has the prototypes and there should be a source code file msgqueue.c, which should have the functions and data types defined.

Write a multithreaded program which links with this message queue library and uses the message queue for communication between multiple threads. In this program there are > 1 writer threads which write into the message queue and there are > 1 reader threads which read from the queue.*/
