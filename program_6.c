 ...
 * The application will accept a filename from command line.
 * The file has strings separated by comma, stored in it. The application will read the strings from the file and store it in a suitable data structure.
 * If the file has “n” number of strings, the application will create “n” number of threads and provide the data structure to every thread.
 * Each thread will access the “ith” string in the data structure, were “I” is the index at which the thread was created.
 * Each thread will find the number of vowels in the string and add the vowel count to the global variable “vowel_count” which has an initial value 0.
